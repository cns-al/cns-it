...
import { ArrowLeft, Copy, Check, RefreshCw, Zap, Lock } from 'lucide-react';
...
export const toolComponents: Record<string, { name: string; component: any }> = {
    ...
    'vault': { name: 'SSH Key & Secret Vault', component: VaultTool },
};
...
function VaultTool() {
  ...
  const [entries, setEntries] = useState<Array<{id: number, name: string, type: 'ssh-key' | 'password' | 'secret', value: string, created: string}>>([]);
  const [newName, setNewName] = useState('');
  const [newType, setNewType] = useState<'ssh-key' | 'password' | 'secret'>('ssh-key');
  const [newValue, setNewValue] = useState('');
  const [editingId, setEditingId] = useState<number | null>(null);
  const [showValue, setShowValue] = useState<Record<number, boolean>>({});

  // Simple XOR-based encryption (client-side only, for convenience)
  ...
  const decrypt = (encrypted: string, key: string): string => {
    try {
      ...
      return '[Decryption failed - wrong master key]';
    }
  };

  const unlock = () => {
    if (!masterKey.trim()) {
      toast.error('Enter a master key');
      return;
    }
    // Test decryption of existing entries
    const stored = localStorage.getItem('cnsit_vault');
    if (stored) {
      try {
        const parsed = JSON.parse(stored);
        let canUnlock = true;
        for (const entry of parsed) {
          const test = decrypt(entry.value, masterKey);
          if (test.startsWith('[Decryption failed')) {
            canUnlock = false;
            break;
          }
        }
        if (canUnlock) {
          ...
          setEntries(parsed.map((e: any) => ({
            ...e,
            value: decrypt(e.value, masterKey)
          })));
        ...
        toast.error('Corrupted vault data');
      }
    } else {
      setUnlocked(true);
      setEntries([]);
  ...
  const lock = () => {
    ...
    const encrypted = entries.map(e => ({
      ...e,
      value: encrypt(e.value, masterKey)
    }));
    localStorage.setItem('cnsit_vault', JSON.stringify(encrypted));
    setUnlocked(false);
    setEntries([]);
    setMasterKey('');
  ...
  const save = () => {
    ...
    if (editingId !== null) {
      setEntries(entries.map(e => e.id === editingId ? { ...e, name: newName, type: newType, value: newValue } : e));
      setEditingId(null);
  ...
  const exportVault = () => {
    const data = entries.map(e => `${e.name} (${e.type}): ${e.value}`).join('\n');
    downloadFile(data, 'vault-export.txt');
  ...
  if (!unlocked) {
    return (
      <div className="max-w-md mx-auto">
        <div className="rounded-xl border border-dark-200 dark:border-dark-700 bg-white dark:bg-dark-900 p-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-brand-100 dark:bg-brand-900/30">
              <Lock className="h-5 w-5 text-brand-600" />
            </div>
            <div>
              <h3 className="font-semibold text-dark-900 dark:text-dark-100">SSH Key & Secret Vault</h3>
              <p className="text-xs text-dark-400">Encrypted storage for SSH keys, passwords, and secrets</p>
            </div>
          </div>
          <p className="text-sm text-dark-500 mb-4">
            Enter a master key to unlock your vault. All data is encrypted client-side and stored in your browser.
          </p>
          <div className="space-y-3">
            <input
              type="password"
            ...
            />
            <button onClick={unlock} className="btn btn-primary w-full">
              Unlock Vault
            </button>
          </div>
        </div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Lock className="h-5 w-5 text-brand-600" />
          <span className="font-medium text-dark-900 dark:text-dark-100">Vault Unlocked</span>
          <span className="badge badge-brand">{entries.length} entries</span>
        </div>
        <div className="flex gap-2">
          <button onClick={exportVault} className="btn btn-secondary text-xs" disabled={entries.length === 0}>
            Export
          </button>
          <button onClick={lock} className="btn btn-secondary text-xs">
            Lock
          </button>
        </div>
      </div>
      ...
      {/* Add Entry Form */}
      <div className="rounded-xl border border-dark-200 dark:border-dark-700 bg-white dark:bg-dark-900 p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
          <input
            ...
            placeholder="Name (e.g., Production Server)"
          ...
          <div className="flex gap-2">
            <button onClick={save} className="btn btn-primary text-sm flex-1">
              {editingId !== null ? 'Update' : 'Save'}
            </button>
            {editingId !== null && (
              <button onClick={() => { setEditingId(null); setNewName(''); setNewValue(''); }} className="btn btn-secondary text-sm px-3">
                Cancel
              </button>
            )}
          </div>
        ...
        <textarea
          ...
          placeholder={newType === 'ssh-key' ? '-----BEGIN OPENSSH PRIVATE KEY-----\n...' : 'Enter secret value...'}
          className="input font-mono text-sm resize-none"
          rows={newType === 'ssh-key' ? 6 : 2}
        />
      </div>
      ...
      {entries.length === 0 ? (
        <div className="text-center py-8 text-dark-400">
          ...
          <p className="text-sm">No entries yet. Add your first SSH key or secret above.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {entries.map(entry => (
            <div key={entry.id} className="rounded-xl border border-dark-200 dark:border-dark-700 bg-white dark:bg-dark-900 p-4">
              <div className="flex items-start justify-between mb-2">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-sm text-dark-900 dark:text-dark-100">{entry.name}</span>
                    <span className={`badge ${entry.type === 'ssh-key' ? 'badge-brand' : entry.type === 'password' ? 'badge-warning' : 'badge-danger'}`}>
                      {entry.type}
                    </span>
                  </div>
                  <span className="text-xs text-dark-400">Added {entry.created}</span>
                </div>
                <div className="flex gap-1">
                  <button
                    onClick={() => setShowValue({ ...showValue, [entry.id]: !showValue[entry.id] })}
                    className="btn btn-secondary text-xs px-2 py-1"
                  >
                    {showValue[entry.id] ? 'Hide' : 'Show'}
                  </button>
                  <button
                    onClick={() => { setEditingId(entry.id); setNewName(entry.name); setNewType(entry.type); setNewValue(entry.value); }}
                    className="btn btn-secondary text-xs px-2 py-1"
                  >
                    Edit
                  </button>
                  <button onClick={() => remove(entry.id)} className="btn btn-secondary text-xs px-2 py-1 text-red-600 hover:text-red-700">
                    Delete
                  </button>
                </div>
              </div>
              {showValue[entry.id] && (
                <div className="mt-2 p-2 rounded-lg bg-dark-50 dark:bg-dark-800">
                  <pre className="text-xs font-mono text-dark-600 dark:text-dark-400 whitespace-pre-wrap break-all">
                    {entry.type === 'ssh-key' ? entry.value : entry.value.substring(0, 20) + '•'.repeat(Math.max(0, entry.value.length - 20))}
                  </pre>
                  {entry.type !== 'ssh-key' && (
                    <button
      ...
      <div className="rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 p-3 text-xs text-amber-700 dark:text-amber-400">
        <strong>Warning:</strong> This vault uses client-side encryption. If you lose your master key, all data is unrecoverable. For production use, consider a dedicated password manager.
      </div>
    </div>
  );
}
